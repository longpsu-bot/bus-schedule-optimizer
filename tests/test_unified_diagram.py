...
def test_combined_demand_chart_stays_combined() -> None:
    ...
    assert all("combined" in value for value in figure.layout.xaxis.categoryarray)
    assert all(
        "outbound" not in value and "inbound" not in value
...
def test_departure_chart_uses_exact_b_and_accepted_c_departures(
    ...
    for scenario, label in ((scenario_b, "B"), (scenario_c, "C")):
        for direction in ("outbound", "inbound"):
            trace = _trace(figure, f"{label} · {direction}")
            assert list(trace.x) == [
...
def test_c_departure_hover_preserves_source_shift_regime_and_vehicle(
    ...
    scenario_c = accepted_presentation.scenario("C")
    assert scenario_c is not None
    trace = _trace(figure, "C · outbound")
    ...
    for customdata, trip in zip(trace.customdata, outbound, strict=True):
        assert customdata[6] == trip.source_b_trip_id
        assert customdata[8] == trip.shift_minutes
        assert customdata[9] == trip.headway_regime_id
        assert customdata[10] == trip.change_reason
        assert customdata[11] == trip.vehicle_assignment

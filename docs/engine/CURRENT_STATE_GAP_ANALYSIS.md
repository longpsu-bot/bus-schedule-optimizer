...
|---|---|---|---|
| `models.py` | useful dataclasses for scenarios, trips, blocks, regimes, traces | reusable after refactor | add normalized A/B/demand envelopes, separate fleet concepts, resolution/lock/problem/solution records, normative statuses |
...
| `fleet.py` | deterministic greedy two-terminal assignment with runtime/turnaround/location | reusable after refactor/validator oracle | not a proof-oriented global solver; current service infers active fleet instead of ingesting approved and available counts separately |
| `service.py` | protects B immutability and asserts strong C locks/fingerprint | reusable after refactor | conflates inferred minimum/declared IDs into active fleet; lacks B disposition and `ScheduleProblemV1`/solver interface |
...
4. **Fleet ambiguity:** approved active fleet and available limit are not input fields. `service.py` derives an “active” count from calculated minimum and declared vehicle IDs, conflating three meanings.
...
No evidence was found that C itself silently changes B parameters: current assertions preserve parameter object equality, total/directional counts, endpoints, per-trip runtime, active fleet IDs/count, and B→C identity. The separate `C2` expansion is visibly labeled, but it is not part of the Contract V1 C definition and needs a future product decision.
